...
const analyzeFood = async (req, res) => {
  try {
    ...
    try {
      ...
      foodName = (analyzeData.category && analyzeData.category.name) ? analyzeData.category.name : 'Unknown Food';
      confidence = Math.round(((analyzeData.category && analyzeData.category.probability) ? analyzeData.category.probability : 0.8) * 100);
    } catch (apiErr) {
